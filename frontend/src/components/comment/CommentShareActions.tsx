...
function CommentShareActions({ comment, matchLabel, className = 'mt-3' }: CommentShareActionsProps) {
  ...
  return (
    <div className={className}>
      ...
      >
        <img src={shareIcon} alt="" className="h-4 w-4 object-contain" aria-hidden="true" />
      </button>
          ...
          <div className="w-full max-w-sm rounded-[1.4rem] border border-[var(--line)] bg-[var(--panel)] p-5 shadow-[var(--shadow)]">
            <div className="flex items-start justify-between gap-4">
              ...
              <button
                ...
                className="rounded-full border border-[var(--line)] px-2.5 py-1 text-sm font-bold text-[var(--muted-strong)] transition hover:text-[var(--text)]"
                aria-label="Fermer"
              >
                ×
              </button>
            ...
            <div className="mt-5 space-y-2">
              <button type="button" onClick={() => void copyLink()} className={optionClass}>
                <ShareOptionIcon type="copy" />
                Copier le lien
              </button>
