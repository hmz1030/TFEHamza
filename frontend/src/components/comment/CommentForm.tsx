...
function CommentForm({ matchId, parentId = null, placeholder = 'Ton commentaire', buttonLabel = 'Commenter', onCreated }: CommentFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3 rounded-[1.6rem] border border-[var(--line)] bg-[rgba(17,27,40,0.72)] p-4">
      ...
      <button type="submit" disabled={loading || !content.trim()} className={parentId ? 'text-action-button text-sm' : 'rounded-full bg-[var(--accent)] px-5 py-2.5 text-sm font-semibold text-[var(--bg-deep)] transition hover:bg-[var(--accent-strong)] disabled:opacity-60'}>
