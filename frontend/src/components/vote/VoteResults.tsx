...
function VoteResults({ votes, players }: VoteResultsProps) {
  ...
  if (results.length === 0) {
    return <div className="rounded-[1.6rem] 
    border border-[var(--line)] 
    bg-[rgba(17,27,40,0.6)] 
    p-5 
    text-sm 
    text-[var(--muted)]
    text-center">
      Aucun joueur disponible pour afficher les votes.</div>
  }
  ...
  if (votes.length === 0) {
    ...
    text-sm 
    text-[var(--muted)]
    text-center">Aucun vote pour le moment.</div>
  }
            ...
            <div className="flex items-center gap-3">
              {player.image ? (
                <img
                  src={player.image}
                  alt={player.name}
                  className="h-10 w-10 rounded-full border border-[var(--line)] bg-[rgba(255,255,255,0.04)] object-cover"
                  loading="lazy"
                />
              ) : (
                <div className="h-10 w-10 rounded-full border border-[var(--line)] bg-[rgba(255,255,255,0.04)]" />
              )}
              <p className="text-sm font-semibold text-[var(--text)]">{player.name}</p>
            </div>
            <p className={`text-sm font-bold ${index === 0 ? 'text-[var(--accent-strong)]' : 'text-[var(--muted-strong)]'}`}>
              {player.total} vote{player.total > 1 ? 's' : ''}
            </p>
          </div>
